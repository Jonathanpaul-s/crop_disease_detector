...
try:
    ...
    st.error("❌ Model not found. Please ensure 'new_crop_disease_detector.pkl' is in the same folder.")
    st.stop()

# App title
st.set_page_config(page_title="🌿 Manual Crop Prediction", layout="centered")
st.title("🌿 Manual Crop Disease Prediction")
st.write("Enter multiple crop readings below to predict diseases.")

# Number of rows user wants to enter
num_rows = st.number_input("How many data rows do you want to enter?", min_value=1, max_value=20, value=3)
...
if st.button("🚀 Predict Disease"):
    try:
        ...
        st.download_button(
            ...
            data=csv_output,
            file_name="manual_predictions.csv",
            mime="text/csv"
        )
    except Exception as e:
