...
def _sync_farm_dataset(
    ...
    """
    Keep Copilot actions and normal Smart Farm AI
    features on the SAME dataframe.

    Returns True only when the dataset was
    ...
    # --------------------------------------------------------
    # Try normal application storage first
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Update live Streamlit state ONLY after successful save
    # --------------------------------------------------------
...
# ============================================================
# 🤖 SMART FARM AI COPILOT — SALE ACTION CONTROLLER
# ============================================================

def farm_ai_format_money(
    value
):
    """
    Format money using Smart Farm AI's country currency
    formatter when available.
    """

    formatter = globals().get(
        "format_farm_money"
    )
...
def farm_ai_handle_pending_action_message(
    ...
    if farm_ai_is_cancellation(
        ...
        return {
            ...
            "ok": True,
            "message": (
                "Okay. I cancelled that farm action "
...
def farm_action_chatbot_ui(
    ...
    with st.container(
        ...
        if st.session_state.get(
            ...
            if not archived_chats:
                ...
                st.info(
                    "No previous Copilot conversations yet."
                )
    ...
    pending_response = (
        farm_ai_handle_pending_action_message(
            command
        )
    )

    if pending_response.get(
        "handled",
        False
    ):
...
def farmer_command_centre_ui():
    ...
    # ========================================================
    # FALL BACK TO FIRST ACTIVE FARM
    # ========================================================
    ...
    # Prefer farm-scoped results first
    # ========================================================
    ...
    recommended_features = (
        st.session_state.get(
            f"recommended_features_{farm_id}"
        )
        or st.session_state.get(
            "recommended_features"
        )
    ...
    # Uses the same owner + farm scoped engine as Copilot
    # ========================================================
    ...
    try:
        ...
        financial_result = (
            farm_action_calculate_profit()
        )

    except Exception as error:

        st.session_state[
            "command_centre_financial_error"
        ] = str(
            error
        )
    ...
    if active_farms:
        ...
        for index, farm in enumerate(
            ...
            farm_option_ids.append(
                candidate_id
            )

        current_index = 0

        if farm_id in farm_option_ids:

            current_index = (
                farm_option_ids.index(
                    farm_id
                )
            )
        ...
        if (
            ...
            try:
                ...
                st.session_state[
                    f"recommended_features_{selected_id}"
                ] = new_recommendations
...
# ========================================================
    ...
    if active_alert_count:
        ...
        briefing_parts.append(
            f"{active_alert_count} active alert(s)"
        )
    ...
    if pa_priority_actions:
        ...
        briefing_parts.append(
            f"{len(pa_priority_actions)} precision action(s)"
        )
    ...
    if briefing_parts:
        ...
        st.info(
            ...
            "before generating evidence-based priorities."
        )
    ...
    # ========================================================
    # SMART FARM AI COPILOT
    # FROZEN WORKING VERSION
    # ========================================================
    ...
    with weather_col:
        ...
        if climate_risks:
            ...
            st.caption(
                "No current climate-risk output "
                "is available from the CSA engine."
            )

        else:
            st.caption(
                "CSA climate intelligence "
                "is awaiting farm data."
            )
    ...
    if (
        ...
        for action in adaptation_actions:
            ...
            if (
                action not in priorities
                and len(priorities) < 4
            ):
